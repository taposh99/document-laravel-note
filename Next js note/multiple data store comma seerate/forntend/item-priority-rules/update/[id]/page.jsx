...
import { ITEM_CATEGORY_API_ENDPOINTS } from '../../../SettingsApiEndPoints';
...
const ConversionUpdate = (props) => {
    const updateUrl = `${ITEM_PRIORITY_RULES_API_ENDPOINTS.ITEM_PRIORITY_RULES_UPDATE}/${props.params.id}`;
    const { data: original, isLoading, update } = useCrudService(updateUrl, {});
    ...
    // Initial values for the form based on the original data
    ...
    useEffect(() => {
        if (original?.data) {
            setInitialValues({
                company_id: original?.data?.company_id || '9f694334-8ece-41a4-8d68-a7fc8d0782d2',
                rule: original?.data?.rule || '',
                item_category_id: original?.data?.item_categories?.map(item => item.id) || [], // Map item category names to IDs
                row_status: original?.data?.row_status === 1 ? '1' : '0',
            });
        }
    }, [original]);
    ...
    const handleSubmit = async (values, { setErrors, setSubmitting }) => {
        const response = await update(values, { url: updateUrl });
        if ([HTTP_CREATED, HTTP_OK].includes(response.status)) {
            await notify("Item updated successfully", SUCCESS);
            setSubmitting(false);
            goToList();
    ...
    const fetchResponseInputType = useCallback(async () => {
        const formInformation = await fetchData(
            ...
            {
                code: "wh_settings_item_priority_rules",
                table_row_id: "",
            }
        );
    ...
    return (
        <>
            {isLoading ? <Loading /> :
                <Grid container spacing={2} py={2} margin="auto" justifyContent="center" alignItems="center">
                    <Grid item xs={12}>
                        <PageTitle title={itemPriorityRulesManagement?.editTitle}
